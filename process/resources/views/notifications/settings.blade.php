...
<div class="page-header">
  <div>
    ...
    <p class="page-subtitle">Contrôlez vos canaux, priorités et routage d'alertes.</p>
  </div>
  <div style="display:flex;gap:10px">
    <button onclick="saveSettings()" class="btn btn-primary" id="save-prefs-btn">
      <i data-lucide="save" style="width:15px;height:15px"></i> Sauvegarder les préférences
    </button>
  </div>
</div>
...
<div id="save-toast" style="display:none;position:fixed;bottom:24px;right:24px;background:var(--success);color:#fff;padding:14px 20px;border-radius:var(--radius);font-size:13px;font-weight:600;box-shadow:var(--shadow-lg);z-index:9999;display:flex;align-items:center;gap:8px">
  <i data-lucide="check-circle-2" style="width:18px;height:18px"></i> Préférences sauvegardées !
</div>
...
<div class="settings-layout">
  ...
  <div class="settings-nav">
    <a href="{{ route('profile.edit') }}" class="settings-nav-item">
      <i data-lucide="user" style="width:15px;height:15px"></i> Général
    </a>
    ...
    <a href="{{ route('profile.security') }}" class="settings-nav-item">
      <i data-lucide="clock" style="width:15px;height:15px"></i> Activité & Sessions
    </a>
  </div>
  ...
  <div>
    ...
    <div class="settings-card">
      ...
      <p class="subtitle">Choisissez comment vous souhaitez être notifié.</p>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px">
        ...
        <div class="channel-card">
          ...
          <div style="flex:1">
            <div style="font-size:13px;font-weight:700;margin-bottom:2px">Email</div>
            <div style="font-size:11px;color:var(--text-muted)">Mises à jour importantes</div>
          </div>
        ...
        <div class="channel-card">
          ...
          <div style="flex:1">
            <div style="font-size:13px;font-weight:700;margin-bottom:2px">Push</div>
            <div style="font-size:11px;color:var(--text-muted)">Notifications mobile & bureau</div>
          </div>
          <label class="toggle-switch">
        ...
        <div class="channel-card">
          ...
          <div style="flex:1">
            <div style="font-size:13px;font-weight:700;margin-bottom:2px">In-App</div>
            <div style="font-size:11px;color:var(--text-muted)">Notifications dans le dashboard</div>
          </div>
          <label class="toggle-switch">
    ...
    {{-- Rule Preferences --}}
    <div class="settings-card">
      <h3>Règles de notification</h3>
      <p class="subtitle">Configurez la priorité de chaque type d'alerte.</p>

      @php
        $rules = [
          ['name' => 'notif_security',      'label' => 'Alertes de sécurité',    'desc' => 'Nouvelles connexions, changements de mot de passe', 'priority' => 'HIGH',   'priority_color' => 'var(--danger)',  'priority_bg' => 'var(--danger-bg)'],
          ['name' => 'notif_grades',        'label' => 'Notes & Bulletins',       'desc' => 'Nouvelles notes, bulletins disponibles',            'priority' => 'HIGH',   'priority_color' => 'var(--danger)',  'priority_bg' => 'var(--danger-bg)'],
          ['name' => 'notif_absences',      'label' => 'Absences',                'desc' => 'Nouvelles absences enregistrées',                   'priority' => 'MEDIUM', 'priority_color' => 'var(--warning)', 'priority_bg' => 'var(--warning-bg)'],
          ['name' => 'notif_payments',      'label' => 'Paiements & Facturation', 'desc' => 'Reçus, rappels de paiement, nouvelles factures',    'priority' => 'HIGH',   'priority_color' => 'var(--danger)',  'priority_bg' => 'var(--danger-bg)'],
          ['name' => 'notif_announcements', 'label' => 'Annonces',                'desc' => 'Nouvelles annonces de l\'établissement',            'priority' => 'MEDIUM', 'priority_color' => 'var(--warning)', 'priority_bg' => 'var(--warning-bg)'],
          ['name' => 'notif_messages',      'label' => 'Messages',                'desc' => 'Nouveaux messages privés et de groupe',             'priority' => 'LOW',    'priority_color' => 'var(--info)',    'priority_bg' => 'var(--info-bg)'],
        ];
      @endphp
    ...
    <div style="display:flex;justify-content:flex-end;gap:10px">
      <a href="{{ route('notifications.index') }}" class="btn btn-outline">← Retour aux notifications</a>
      <button onclick="saveSettings()" class="btn btn-primary">
        <i data-lucide="save" style="width:15px;height:15px"></i> Sauvegarder
      </button>
    </div>
...
async function saveSettings() {
  ...
  const r = await fetch('{{ route("notifications.settings.save") }}', {
    method: 'POST',
    headers: {
