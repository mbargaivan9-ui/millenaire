{{--
    | public/landing.blade.php — Page d'accueil publique
    --}}

@extends('layouts.public')
@section('title', $settings->platform_name ?? 'Millénaire Connect')
    ...
    <div class="container" style="max-width:1200px;margin:0 auto;padding:0 1.5rem;position:relative">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4rem;align-items:center">
            <div>
                ...
                <img src="{{ asset($settings->logo_path) }}" style="height:64px;margin-bottom:1.5rem;border-radius:12px">
                @endif
                <h1 style="font-size:3rem;font-weight:900;line-height:1.1;margin-bottom:1rem">
                    {{ $settings->platform_name ?? 'Millénaire Connect' }}
                </h1>
                <p style="font-size:1.15rem;opacity:.88;margin-bottom:2rem;line-height:1.7">
                    {{ $settings->slogan ?? ($isFr ? 'La plateforme digitale complète du Collège Millénaire Bilingue — Bulletins, présences, paiements et communication en temps réel.' : 'The complete digital platform of Collège Millénaire Bilingue — Report cards, attendance, payments and real-time communication.') }}
                </p>
            ...
            <div style="text-align:center">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                    @foreach([
                        ['🎓', $stats['students'] ?? '500+', $isFr ? 'Élèves' : 'Students'],
                        ['👨‍🏫', $stats['teachers'] ?? '30+',  $isFr ? 'Enseignants' : 'Teachers'],
                        ['📊', $stats['bulletins'] ?? '2000+', $isFr ? 'Bulletins générés' : 'Report cards'],
                        ['💬', '99%', $isFr ? 'Satisfaction' : 'Satisfaction'],
                    ] as [$icon, $val, $lbl])
                    <div style="background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.15);border-radius:16px;padding:1.5rem;backdrop-filter:blur(4px);text-align:center">
                        <div style="font-size:2rem;margin-bottom:.25rem">{{ $icon }}</div>
                        <div style="font-size:1.8rem;font-weight:900;margin-bottom:.2rem">{{ $val }}</div>
                        <div style="font-size:.78rem;opacity:.8;font-weight:600">{{ $lbl }}</div>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="container" style="max-width:1200px;margin:0 auto;padding:0 1.5rem">
        <div style="text-align:center;margin-bottom:3rem">
            <h2 style="font-size:2rem;font-weight:900;color:#0f172a;margin-bottom:.75rem">
                {{ $isFr ? 'Tout ce dont votre école a besoin' : 'Everything your school needs' }}
            </h2>
            <p style="font-size:1rem;color:#64748b;max-width:600px;margin:0 auto">
                {{ $isFr ? 'Une plateforme complète pour digitaliser la gestion scolaire' : 'A complete platform to digitize school management' }}
            </p>
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1.5rem">
            @foreach([
                ['📊', $isFr ? 'Bulletins Numériques' : 'Digital Report Cards', $isFr ? 'Générés automatiquement avec signature et QR code d\'authentification.' : 'Auto-generated with signature and QR authentication code.'],
                ['📅', $isFr ? 'Emplois du Temps' : 'Timetables', $isFr ? 'Gestion des emplois du temps pour toutes les classes et enseignants.' : 'Schedule management for all classes and teachers.'],
                ['✅', $isFr ? 'Appel & Présences' : 'Attendance', $isFr ? 'Appel numérique avec notifications parents en temps réel.' : 'Digital roll call with real-time parent notifications.'],
                ['💰', $isFr ? 'Paiements Mobile Money' : 'Mobile Money Payments', $isFr ? 'Orange Money & MTN MoMo intégrés. Reçus PDF automatiques.' : 'Integrated Orange Money & MTN MoMo. Auto PDF receipts.'],
                ['💬', $isFr ? 'Messagerie Temps Réel' : 'Real-time Messaging', $isFr ? 'Chat entre enseignants, parents et administration.' : 'Chat between teachers, parents and administration.'],
                ['📚', $isFr ? 'E-Learning' : 'E-Learning', $isFr ? 'Ressources pédagogiques, quiz et devoirs en ligne.' : 'Teaching resources, quizzes and online assignments.'],
            ] as [$icon, $title, $desc])
            <div style="background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:1.5rem;transition:all .2s ease"
                ...
                <h3 style="font-size:1rem;font-weight:800;color:#0f172a;margin-bottom:.5rem">{{ $title }}</h3>
                <p style="font-size:.85rem;color:#64748b;line-height:1.6;margin:0">{{ $desc }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<section style="padding:4rem 0;background:#fff">
    <div class="container" style="max-width:1200px;margin:0 auto;padding:0 1.5rem">
        <h2 style="font-size:1.5rem;font-weight:900;color:#0f172a;margin-bottom:1.5rem;text-align:center">
            {{ $isFr ? 'Dernières Annonces' : 'Latest Announcements' }}
        </h2>
...
<section style="padding:4rem 0;background:#f8fafc">
    <div class="container" style="max-width:1200px;margin:0 auto;padding:0 1.5rem">
        <h2 style="font-size:1.5rem;font-weight:900;text-align:center;color:#0f172a;margin-bottom:2rem">
            {{ $isFr ? 'Ce qu\'ils en pensent' : 'What they say' }}
        </h2>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1.25rem">
            ...
            <div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:1.5rem">
                ...
                <p style="font-size:.85rem;color:#475569;line-height:1.7;margin-bottom:1rem;font-style:italic">
                    "{{ app()->getLocale() === 'fr' ? $t->content_fr : ($t->content_en ?? $t->content_fr) }}"
                </p>
...
<section style="padding:4rem 0;background:linear-gradient(135deg,var(--primary),var(--primary-dark));color:#fff;text-align:center">
    <div class="container" style="max-width:600px;margin:0 auto;padding:0 1.5rem">
        <h2 style="font-size:1.75rem;font-weight:900;margin-bottom:.75rem">
            {{ $isFr ? 'Prêt à rejoindre la plateforme ?' : 'Ready to join the platform?' }}
        </h2>
        <p style="opacity:.88;margin-bottom:2rem">
            {{ $isFr ? 'Connectez-vous avec vos identifiants fournis par l\'établissement.' : 'Sign in with the credentials provided by the school.' }}
        </p>
