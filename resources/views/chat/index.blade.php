        ...
        <div class="chat-conv-list" id="convList">
            @forelse($conversations as $conv)
                @php
                    ...
                    $avatar = $other?->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode(substr($displayName,0,2)).'&background=1abc9c&color=fff&size=80';
            ...
            <div class="chat-main-header" id="chatHeader">
                @if(isset($activeConversation))
                    @php
                        ...
                        $hAvatar = $hOther?->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode(substr($hName,0,2)).'&background=1abc9c&color=fff&size=80';
...
async function loadUsers(search) {
    ...
    try {
        const url = `/chat/search-users?q=${encodeURIComponent(search)}`;
        ...
        if (!data.users) {
            throw new Error('Invalid response: no users field');
        }
        ...
        list.innerHTML = `<div style="padding:16px;text-align:center;color:#ef4444;font-size:.82rem">
            <div style="margin-bottom:8px">Erreur de chargement</div>
            <small style="display:block;margin-top:4px;color:#6b7280">${escHtml(errorMsg)}</small>
        </div>`;
    }
}
