...
class SchoolPayController extends Controller
{
    ...
    public function parentIndex(Request $request): View
    {
        ...
        // Récupérer les enfants du parent connecté via la relation Guardian
        if ($user->guardian) {
            $students = $user->guardian->students()->with('classe')->get();
        } elseif ($user->role === 'parent') {
            // Si l'utilisateur est parent mais n'a pas de guardian record,
            // chercher les students directement via la relation guardians
            $students = Student::whereHas('guardians', fn($q) => $q->where('user_id', $user->id))
                ->with('classe')->get();
        }
